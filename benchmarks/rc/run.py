"""Run the rc benchmark on circulax + VACASK + ngspice; update README table.
...
def run_ngspice() -> dict:
    ...
    if proc.returncode != 0:
        return {"simulator": "ngspice", "status": f"rc={proc.returncode}",
                "wall_s": wall}
    # ngspice rusage 'Total elapsed time' on its own line.
    m = re.search(r"Total elapsed time[^\d]*([\d.]+)", proc.stdout)
    # ngspice reports `Accepted timepoints = 1006013` (integer) and
    # also `length(time) = 1.006013e+06` (sci-notation — don't parse
    # with \d+, you'll get the leading 1).
...
def run_circulax() -> dict:
    import bench_circulax as cxmod  # local runner next to this run.py
    r = cxmod.run()
    return {
